...
SeparateMultiTaskLinearEncoder::SeparateMultiTaskLinearEncoder(const vector<int> & vocab_size, int wordrep_size,
...
SeparateMultiTaskLinearEncoder* SeparateMultiTaskLinearEncoder::Read(std::istream & in, dynet::Model & model) {
  string vocab_sizes;
  ...
  SeparateMultiTaskLinearEncoder * ret = new SeparateMultiTaskLinearEncoder(v_sizes, wordrep_size, BuilderSpec(hidden_spec), unk_id, model);
  if(reverse == "rev") ret->SetReverse(true);
  return ret;
}
void SeparateMultiTaskLinearEncoder::Write(std::ostream & out) {
...
void SeparateMultiTaskLinearEncoder::SetDropout(float dropout) { 
