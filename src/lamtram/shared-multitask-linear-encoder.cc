...
SharedMultiTaskLinearEncoder::SharedMultiTaskLinearEncoder(const vector<int> & vocab_size, int wordrep_size,
...
SharedMultiTaskLinearEncoder* SharedMultiTaskLinearEncoder::Read(std::istream & in, dynet::Model & model) {
  string vocab_sizes;
  ...
  if(version_id != "sharedmultilinenc_001")
  ...
  SharedMultiTaskLinearEncoder * ret = new SharedMultiTaskLinearEncoder(v_sizes, wordrep_size, BuilderSpec(hidden_spec), unk_id, model);
  if(reverse == "rev") ret->SetReverse(true);
  return ret;
}
void SharedMultiTaskLinearEncoder::Write(std::ostream & out) {
  out << "sharedmultilinenc_001 ";
  out << vocab_sizes_[0];
  for (int i = 1; i < vocab_sizes_.size(); i++) {
    out << "|" << vocab_sizes_[i] << endl;
  }
